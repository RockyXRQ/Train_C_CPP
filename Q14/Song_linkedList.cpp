...
void song_info_collection() {
    printf("请输入要记录的歌曲的名字：");
    while (s_gets(input, 30) &&
           input[0] != '\0') {  //输入为空值的那次循环将结束获取信息
        current = (struct song*)malloc(sizeof(
            struct song));  //循环的每一次开始，current都会指向新位置的内存
        ...
        if (head == NULL)    //链表信息的获取从第一个获取
            ...
            prev->next =
                current;  //从一个head存在之后，每一个结构体内的next指向下一个结构体
